...
function onUpdatePost()
	...
	if ENABLED == true then
		...
		if swapDirections == true then
			...
			if bfIconOnly == true then -- this works only with the size I have it set to, if changed, this will probably break | by break I mean the icon'll be off
				P1Mult = getProperty('healthBar.y') - ((getProperty('healthBar.width') * (getProperty('healthBar.percent') / getProperty('healthBar.scale.y') - 0.005) * 0.01) + (150 * getProperty('iconP1.scale.x') - 150)) + 270
			else
				P1Mult = getProperty('healthBar.y') - ((getProperty('healthBar.width') * getProperty('healthBar.percent') * getProperty('healthBar.scale.y') * 0.01) + (150 * getProperty ('iconP1.scale.x') - 150)) + 255
			end
